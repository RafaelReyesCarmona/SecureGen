...
.cat-filter-btn:first-child {
    border-radius: 15px 8px 0 0;
}
.cat-filter-btn:last-child {
    border-radius: 8px 15px 0 0;
}
@media (max-width: 480px) {
    .cat-filter-btn span:not(.cat-emoji) { display: none; }
    .cat-filter-btn { padding: 8px 4px; }
}

/* Category badge (above password name) */
.badge-category {
    display: inline-flex;
    align-items: center;
    gap: 3px;
...
.badge-category:hover { opacity: 0.8; }
.badge-cat-web   { background: #dbeafe; color: #1e40af; }
.badge-cat-app   { background: #fce7f3; color: #9d174d; }
.badge-cat-local { background: #dcfce7; color: #166534; }
.badge-cat-key   { background: #fef9c3; color: #854d0e; }
.badge-cat-none  { background: var(--bg-tertiary); color: var(--text-muted); border: 1px dashed var(--border); }

/* Dark mode category badge overrides */
@media (prefers-color-scheme: dark) {
    .badge-cat-web   { background: #1e3a5f; color: #93c5fd; }
    .badge-cat-app   { background: #4a1942; color: #f9a8d4; }
    .badge-cat-local { background: #14421e; color: #86efac; }
    .badge-cat-key   { background: #422006; color: #fde047; }
}

/* Category quick-select popup */
.cat-popup {
    position: absolute;
    z-index: 9999;
    min-width: 180px;
...
.cat-popup-item.selected {
    background: rgba(90, 158, 238, 0.2);
    color: #5a9eee;
    font-weight: 600;
}
.cat-popup-item .cat-emoji {
    font-size: 16px;
    flex-shrink: 0;
}

/* Category selector in forms (edit/add modal) */
.cat-selector-wrap {
    display: flex;
    align-items: center;
    gap: 10px;
...
.cat-selector-btn:hover { border-color: var(--border-active); }
.cat-selector-btn .cat-emoji { font-size: 18px; }
...
@media (max-width: 768px) {
    ...
    #keys-table th:nth-child(1), #keys-table td:nth-child(1) {
        width: 20px !important;
        padding: 5px 2px !important;
    }

    #keys-table th:nth-child(2), #keys-table td:nth-child(2),
    #keys-table th:nth-child(3), #keys-table td:nth-child(3),
    #keys-table th:nth-child(4), #keys-table td:nth-child(4) {
        ...
        <form id="add-password-form">
            ...
            <div class="cat-selector-wrap">
                ...
                <button type="button" class="cat-selector-btn" id="add-cat-btn"
                    onclick="openCatPopup(event, this, (catId) => { _addCatValue = catId; updateAddCatBtn(); })">
                  <span class="cat-emoji" id="add-cat-emoji">📋</span>
                  <span id="add-cat-label">All</span>
                </button>
            </div>
            <label class="auto-send-wrap" style="display:flex;align-items:center;gap:8px;margin-top:8px;font-size:0.85em;opacity:0.85;cursor:pointer;">
              <input type="checkbox" id="add-auto-send" name="auto_send" value="1" style="width:16px;height:16px;accent-color:var(--accent);">
              <span data-i18n="passwords.auto_send.label">Auto-send (Enter)</span>
            </label>
            <button type="submit" class="button user-activity" data-i18n="passwords.add.btn">Add Password</button>
        </form>
    </div>
    <div id="cat-filter-container"></div>
    <div class="content-box" style="border-radius: 0 0 15px 15px; margin-top: 0;">
...
<div id="Display" class="tab-content">
    ...
    <div class="form-container">
        ...
        <div id="rtc-section" style="margin-top:20px;padding-top:15px;border-top:1px solid rgba(255,255,255,0.1);">
            ...
            <p id="rtc-board-note" style="font-size:11px;opacity:0.5;margin:6px 0 0 0;"></p>
    ...
    <!-- Display Rotation -->
    <div class="form-container">
        <h4 data-i18n="display.rotation.title">Display Orientation</h4>
        <label for="display-rotation" data-i18n="display.rotation.label">Screen orientation:</label>
        <select id="display-rotation" name="display_rotation" class="user-activity" style="width: 100%; padding: 8px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; margin-bottom: 16px;" onchange="saveDisplayRotation(this.value)">
            <option value="1" data-i18n="display.rotation.normal">Normal (landscape)</option>
            <option value="3" data-i18n="display.rotation.flipped">Flipped (180°)</option>
        </select>
    </div>
    ...
    <div class="form-container">
        ...
        <form id="display-timeout-form">
            <label for="dim-timeout" data-i18n="display.lock.dim.label">Auto-dim (reduce brightness to 20% after):</label>
            <select id="dim-timeout" name="dim_timeout" required class="user-activity" style="width: 100%; padding: 8px; font-size: 14px; border: 1px solid #ccc; border-radius: 4px; margin-bottom: 16px;">
                <option value="5" data-i18n="display.lock.opt.5s">5 seconds</option>
                <option value="10" data-i18n="display.lock.opt.10s">10 seconds</option>
...
<div id="Pin" class="tab-content">
    ...
    <div class="form-container" id="pin-startup-section">
    ...
    <!-- Duress PIN Section -->
    <div class="form-container" id="duress-pin-section" style="margin-top:30px;border-top:1px solid rgba(255,255,255,0.1);padding-top:25px;">
        <h4 data-i18n="pin.duress.title" style="color:#e24a4a;margin-bottom:15px;">Duress PIN</h4>
        <p style="color:#888;font-size:0.9em;margin-bottom:20px;">
            <strong>Security Notice:</strong>
            <span data-i18n="pin.duress.notice">Entering this PIN will permanently destroy all data on the device.</span>
        </p>
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;padding:15px;background:rgba(255,255,255,0.05);border-radius:8px;">
            <div>
                <label for="duress-pin-enabled" data-i18n="pin.duress.label.enable" style="font-size:16px;font-weight:500;margin:0;">Enable Duress PIN</label>
                <p data-i18n="pin.duress.desc" style="margin:5px 0 0 0;font-size:13px;color:rgba(255,255,255,0.6);">A separate PIN that wipes the device when entered</p>
            </div>
            <label class="switch">
                <input type="checkbox" id="duress-pin-enabled" class="user-activity" onchange="toggleDuresspinFields()">
                <span class="slider"></span>
            </label>
        </div>
        <div id="duress-pin-fields" style="display:none;">
            <form id="duress-pin-form">
                <label for="duress-pin" data-i18n="pin.duress.label.new">New Duress PIN (same length as startup PIN):</label>
                <div class="password-input-container">
                    <input type="password" id="duress-pin" name="duress_pin"
                           pattern="\d{4,10}" maxlength="6"
                           placeholder="Enter duress PIN"
                           class="user-activity">
                    <span class="password-toggle" onclick="togglePasswordVisibility('duress-pin',this)">O</span>
                </div>
                <label for="duress-pin-confirm" data-i18n="pin.duress.label.confirm">Confirm Duress PIN:</label>
                <div class="password-input-container">
                    <input type="password" id="duress-pin-confirm" name="duress_pin_confirm"
                           pattern="\d{4,10}" maxlength="6"
                           placeholder="Confirm duress PIN"
                           class="user-activity">
                    <span class="password-toggle" onclick="togglePasswordVisibility('duress-pin-confirm',this)">O</span>
                </div>
                <div style="margin:15px 0;padding:12px;background:rgba(220,53,69,0.15);border:1px solid rgba(220,53,69,0.4);border-radius:6px;">
                    <small style="color:#ff6b6b;font-size:0.85rem;">
                        <strong>&#9888; Warning:</strong>
                        <span data-i18n="pin.duress.important">This PIN must differ from your main PIN. Entry immediately destroys all data.</span>
                    </small>
                </div>
                <button type="submit" data-i18n="pin.duress.btn" class="button user-activity" style="background-color:#dc3545;">Save Duress PIN</button>
            </form>
        </div>
    </div>

    <!-- Hidden Space Section -->
    <div class="form-container" id="hidden-space-section" style="margin-top:30px;border-top:1px solid rgba(255,255,255,0.1);padding-top:25px;">
        <h4 data-i18n="hidden_space_title" style="color:#9b59b6;margin-bottom:15px;">Hidden Space</h4>
        <p class="hint" id="hidden-space-hint" style="color:#888;font-size:0.9em;margin-bottom:20px;">A second encrypted space. Only accessible with a different PIN.</p>
        
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;padding:15px;background:rgba(255,255,255,0.05);border-radius:8px;">
            <div>
                <label for="hidden-space-enabled" data-i18n="hidden_space_label" style="font-size:16px;font-weight:500;margin:0;">Enable Hidden Space</label>
                <p style="margin:5px 0 0 0;font-size:13px;color:rgba(255,255,255,0.6);">Toggle to enable or disable hidden space</p>
            </div>
            <label class="switch">
                <input type="checkbox" id="hidden-space-enabled" class="user-activity" onchange="toggleHiddenSpaceField()">
                <span class="slider"></span>
            </label>
        </div>
        
        <div id="hidden-space-confirm-block" style="display:none;">
            <p class="warning-text" data-i18n="hidden_space_warning" style="color:#ff6b6b;font-size:0.9em;margin-bottom:15px;padding:12px;background:rgba(220,53,69,0.15);border:1px solid rgba(220,53,69,0.4);border-radius:6px;">
                <strong>&#9888; Warning:</strong> Enabling will reboot the device for second PIN setup.
            </p>
        </div>
        
        <button type="button" id="hidden-space-action-btn" class="button user-activity" onclick="submitHiddenSpaceAction()" style="width:100%;margin-bottom:15px;">
            Enable & Setup
        </button>
        
        <div class="form-group toggle-row" id="share-wifi-row" style="display:none;">
            <label data-i18n="hidden_space_share_wifi_label">Share WiFi with hidden space</label>
            <label class="switch">
                <input type="checkbox" id="share-wifi-enabled" class="user-activity" onchange="submitShareWifiChange()">
                <span class="slider"></span>
            </label>
        </div>
        <p class="hint" id="share-wifi-hint" style="display:none;" data-i18n="hidden_space_share_wifi_hint"></p>
    </div>
...
<div id="Settings" class="tab-content">
    ...
    <div class="form-container">
        ...
        <form id="change-password-form">
            ...
            <ul class="password-criteria">
                ...
                <li id="pwd-no-username" data-i18n="settings.pwd.req.nousername">Password must not contain your username</li>
            </ul>
    ...
    <div id="hid-mode-section" class="form-container" style="display:none;">
        <h4 data-i18n="settings.hid.title">HID Send Mode</h4>
        <form id="hid-mode-form">
            <label for="hid-mode-select" data-i18n="settings.hid.label">Default mode when sending password:</label>
            <select id="hid-mode-select" name="hid_mode" required class="user-activity" style="width:100%;padding:8px;font-size:14px;border:1px solid #ccc;border-radius:4px;margin-bottom:10px;">
                <option value="ble" data-i18n="settings.hid.opt.ble">BLE (auto) / USB on BTN2</option>
                <option value="usb" data-i18n="settings.hid.opt.usb">USB (auto) / BLE on BTN1</option>
            </select>
            <p data-i18n="settings.hid.note" style="font-size:12px;opacity:0.6;margin:0 0 10px 0;">Override available via buttons during 3s prompt.</p>
            <button id="save-hid-mode-btn" data-i18n="settings.hid.btn" type="button" class="button user-activity">Save HID Mode</button>
        </form>
    </div>
    <div class="form-container">
    ...
    <div class="form-container">
        ...
        <form id="web-server-settings-form">
            <label for="web-server-timeout" data-i18n="settings.webserver.label">Auto-shutdown on inactivity (WiFi Mode):</label>
        ...
        <div class="form-group">
            ...
            <div class="cat-selector-wrap">
                ...
                <button type="button" class="cat-selector-btn" id="edit-cat-btn"
                    onclick="openCatPopup(event, this, (catId) => { _editCatValue = catId; updateEditCatBtn(); })">
                  <span class="cat-emoji" id="edit-cat-emoji">📋</span>
                  <span id="edit-cat-label">All</span>
                </button>
            </div>
            <label class="auto-send-wrap" style="display:flex;align-items:center;gap:8px;margin-top:8px;font-size:0.85em;opacity:0.85;cursor:pointer;">
              <input type="checkbox" id="edit-auto-send" name="auto_send" value="1" style="width:16px;height:16px;accent-color:var(--accent);">
              <span data-i18n="passwords.auto_send.label">Auto-send (Enter)</span>
            </label>
            <div class="password-strength-container">
                <div class="password-strength-bar">
...
// Helper: Check if current space is B (to skip global config fetches)
function isSpaceB() {
    const hs = CacheManager.get('hidden_space_status');
    return hs && hs.current_space === 'B';
}

let appInitialized = false;
let devicePinLength = 6; // updated from /api/pincode_settings
let boardHasUsbHid = false;
...
function fetchKeys(){
    ...
    if (cachedKeys) {
        console.log('⚡ Using cached keys list, fetching fresh TOTP codes...');
        keysData = cachedKeys;
        ...
        // Force immediate server fetch — cache has no codes or sync state
        updateTOTPCodes(true);
    ...
    .then(data => {
        ...
        const keysForCache = data.map(key => ({
            ...
            p: key.p,
            d: key.d
            // НЕ кешируем code, timeLeft, c - они динамические!
        }));
...
function updateKeysTable(data) {
    ...
    keysData.forEach((key, index) => {
        ...
        const hotpCounter = (key.c !== undefined && key.c !== null) ? key.c : 0;
        const timerCell = isHOTP 
            ? `<td><span id="timer-${index}" style="color:#44ff44;">🔄 ${hotpCounter}</span></td>`
...
function updateTOTPCodes(force = false) {
    ...
    if (force || timeInPeriod <= 1 || !keysData || (hasHOTPOnly && currentTime % 5 === 0)) {
    ...
    data.forEach((key, index) => {
        ...
        if (codeElement && progressElement && timerElement) {
            ...
            if (key.t === 'H') {
                ...
                const cachedCounter = (key.c !== undefined && key.c !== null) ? key.c : 0;
                timerElement.textContent = '🔄 ' + cachedCounter;
...
function formatKeyMetadata(key) {
    ...
    return parts.length > 0
        ? ' <span class="key-badges">' + parts.map(p => '<span class="key-badge">' + p + '</span>').join('') + '</span>'
        : '';
}

function formatPasswordMetadata(pwd, allPasswords) {
    const parts = [];
    
    // DUP: pw_hash matches another entry
    if (pwd.pw_hash && pwd.pw_hash.length > 0) {
        const dupFound = allPasswords.some(p =>
            p !== pwd && p.pw_hash && p.pw_hash === pwd.pw_hash
        );
        if (dupFound) parts.push('DUP');
    }
    
    // Strength (only Weak and Medium — Strong is the default/good state)
    if (pwd.strength === 1) parts.push('Weak');
    else if (pwd.strength === 2) parts.push('Medium');
    else if (pwd.strength === 3) parts.push('Strong');
    
    // PIN: password contains only digits
    if (pwd.password && pwd.password.length > 0 &&
        /^\d+$/.test(pwd.password)) {
        parts.push('PIN');
    }
    
    // NAME: password contains a word from the name (>=3 chars, case-insensitive)
    if (pwd.password && pwd.name) {
        const pwLower = pwd.password.toLowerCase();
        const nameLower = pwd.name.toLowerCase();
        const words = nameLower.match(/[a-z]{3,}/g) || [];
        if (words.some(w => pwLower.includes(w))) {
            parts.push('NAME');
        }
    }
    if (pwd.auto_send) {
        parts.push('<span class="badge" title="' +
            tr('passwords.auto_send.hint') + '">&#x23CE; ' +
            tr('passwords.auto_send.badge') + '</span>');
    }
    
    return parts.length > 0
        ? ' <span class="key-badges">' +
          parts.map(p => '<span class="key-badge">' + p + '</span>').join('') +
          '</span>'
        : '';
}
    ...
    makeAuthenticatedRequest('/api/hotp/generate', {
        ...
        if (data && data.code) {
            if (keysData && keysData[index]) {
                ...
                if (timerEl) timerEl.textContent = '🔄 ' + data.counter;
            }
...
function editPassword(index) {
    ...
    makeAuthenticatedRequest('/api/passwords/get', { method: 'POST', body: new URLSearchParams(formData) })
        .then(async response => {
            ...
            _editCatValue = data.category || '';
            document.getElementById('edit-auto-send').checked = data.auto_send || false;
            updateEditCatBtn();
...
function updateEditCatBtn() {
    const cat = getCatInfo(_editCatValue);
    const emojiEl = document.getElementById('edit-cat-emoji');
    const labelEl = document.getElementById('edit-cat-label');
    const btnEl = document.getElementById('edit-cat-btn');
    if (!emojiEl) return;
    emojiEl.textContent = cat.emoji;
    labelEl.textContent = _editCatValue ? tr(cat.labelKey) : tr('passwords.cat.none');
    btnEl.dataset.cat = _editCatValue;
}

function updateAddCatBtn() {
    const cat = getCatInfo(_addCatValue);
    const emojiEl = document.getElementById('add-cat-emoji');
    const labelEl = document.getElementById('add-cat-label');
    const btnEl = document.getElementById('add-cat-btn');
    if (!emojiEl) return;
    emojiEl.textContent = cat.emoji;
    labelEl.textContent = _addCatValue ? tr(cat.labelKey) : tr('passwords.cat.none');
    btnEl.dataset.cat = _addCatValue;
}
...
function savePasswordEdit() {
    ...
    formData.append('category', _editCatValue);
    if (document.getElementById('edit-auto-send').checked) {
        formData.append('auto_send', '1');
    }
...
let _catPopupAnchor = null;

function openCatPopup(event, anchorEl, callback) {
    event.stopPropagation();
    // Toggle: same button clicked again → just close
    if (_catPopupEl && _catPopupAnchor === anchorEl) {
        closeCatPopup();
        return;
    }
    closeCatPopup();
    _catPopupAnchor = anchorEl;
    
    const currentCat = anchorEl.dataset.cat || '';
    const popup = document.createElement('div');
    popup.className = 'cat-popup';
    popup.id = 'cat-popup-el';
    
    CATEGORIES.forEach(cat => {
        const label = cat.id ? tr(cat.labelKey) : tr('passwords.cat.all');
        const isSelected = cat.id !== '' && cat.id === currentCat;
        const item = document.createElement('div');
        item.className = 'cat-popup-item' + (isSelected ? ' selected' : '');
        item.innerHTML = `<span class="cat-emoji">${cat.emoji}</span>${label}`;
        item.onclick = (e) => {
            e.stopPropagation();
            if (callback) {
                callback(cat.id);
            } else {
                // Quick-change: update badge in table, save immediately
                const idx = parseInt(anchorEl.closest('[data-pwd-index]')?.dataset.pwdIndex ?? '-1');
                if (idx >= 0) quickUpdateCategory(idx, cat.id, anchorEl);
            }
            closeCatPopup();
        };
    ...
    requestAnimationFrame(() => {
        ...
        if (left + pw > vw - 8) popup.style.left = (vw - pw - 8) + 'px';
    });
...
function closeCatPopup() {
    ...
    _catPopupAnchor = null;
}

async function quickUpdateCategory(pwdIndex, newCat, badgeEl) {
    const pwd = passwordsData[pwdIndex];
    if (!pwd) return;
    
    // Optimistic UI update
    pwd.category = newCat;
    const newBadge = document.querySelector('[data-pwd-index="' + pwdIndex + '"] .badge-category');
    if (newBadge) newBadge.outerHTML = catBadgeHTML(newCat, true);
    
    // Send update using same method as savePasswordEdit
    const formData = new FormData();
    formData.append('index', pwdIndex);
    formData.append('name', pwd.name || '');
    formData.append('password', pwd.password || '');
    formData.append('category', newCat);
    formData.append('auto_send', pwd.auto_send ? '1' : '0');
    
    try {
        await makeAuthenticatedRequest('/api/passwords/update', {
            method: 'POST',
            body: new URLSearchParams(formData)
        });
        CacheManager.invalidate('passwords_list');
    } catch(e) {
        showStatus(tr('error') || 'Error saving', true);
        fetchPasswords();
    }
}
...
// ── Category filter bar ───────────────────────────────────────
function renderCategoryFilterBar(containerId) {
    ...
    CATEGORIES.forEach(cat => {
        const label = cat.id === '' ? tr('passwords.cat.all') : tr(cat.labelKey);
        ...
        btn.onclick = () => {
            ...
            renderCategoryFilterBar(containerId);
            fetchPasswords();
        };
        bar.appendChild(btn);
    });
...
function fetchPasswords(){
    ...
    renderCategoryFilterBar('cat-filter-container');
    ...
    if (cachedPasswords) {
        ...
        const visiblePasswords = (activeCategoryFilter === '')
            ? cachedPasswords
            : cachedPasswords.filter(p => (p.category || '') === activeCategoryFilter);
        
        const tbody = document.querySelector('#passwords-table tbody');
        tbody.innerHTML = '';
        visiblePasswords.forEach((password, visibleIndex) => {
            // Find original index in full passwordsData array
            const originalIndex = cachedPasswords.indexOf(password);
            
            const row = tbody.insertRow();
            row.className = 'draggable-row';
            row.draggable = true;
            row.dataset.pwdIndex = originalIndex;
            row.dataset.index = originalIndex;
            row.innerHTML = '<td><span class="drag-handle">::</span></td><td>' + 
                            catBadgeHTML(password.category || '', true) + 
                            '<br>' + password.name + formatPasswordMetadata(password, cachedPasswords) + 
                            '</td><td><button class="button-copy user-activity" onclick="copyPassword(' + originalIndex + ')">' + tr('passwords.btn.copy') + '</button><button class="button-action user-activity" onclick="editPassword(' + originalIndex + ')">' + tr('passwords.btn.edit') + '</button><button class="button-delete user-activity" onclick="removePassword(' + originalIndex + ')">' + tr('passwords.btn.remove') + '</button></td>';
        });
    ...
    .then(async response => {
        ...
        // ⚡ MEMORY: Храним все поля включая category
        passwordsData = data.map(pwd => ({
            name: pwd.name,
            strength: pwd.strength,
            pw_hash: pwd.pw_hash,
            password: pwd.password,
            category:  pwd.category  || '',
            auto_send: pwd.auto_send || false,
            username: pwd.username || '',
            url: pwd.url || ''
        }));
        
        // ⚡ CACHE: Сохраняем данные паролей
        CacheManager.set('passwords_list', passwordsData);
        
        // Filter by active category
        const visiblePasswords = (activeCategoryFilter === '')
            ? passwordsData
            : passwordsData.filter(p => (p.category || '') === activeCategoryFilter);
        
        const tbody = document.querySelector('#passwords-table tbody');
        tbody.innerHTML = '';
        visiblePasswords.forEach((password, visibleIndex) => {
            // Find original index in full passwordsData array
            const originalIndex = passwordsData.indexOf(password);
            ...
            row.innerHTML = '<td><span class="drag-handle">::</span></td><td>' + 
                            catBadgeHTML(password.category || '', true) + 
                            '<br>' + password.name + formatPasswordMetadata(data[originalIndex], data) + 
                            '</td><td><button class="button-copy user-activity" onclick="copyPassword(' + originalIndex + ')">' + tr('passwords.btn.copy') + '</button><button class="button-action user-activity" onclick="editPassword(' + originalIndex + ')">' + tr('passwords.btn.edit') + '</button><button class="button-delete user-activity" onclick="removePassword(' + originalIndex + ')">' + tr('passwords.btn.remove') + '</button></td>';
        });
        initializeDragAndDrop('passwords-table', 'passwords');
    })
...
document.getElementById('add-password-form').addEventListener('submit', function(e) {
    ...
    formData.append('name', name);
    formData.append('password', password);
    formData.append('category', _addCatValue);
    if (document.getElementById('add-auto-send').checked) {
        formData.append('auto_send', '1');
    }
    
    makeAuthenticatedRequest('/api/passwords/add', {method: 'POST', body: formData})
        .then(data => {
            CacheManager.invalidate('passwords_list');
            showStatus(tr('passwords.added'));
            fetchPasswords();
            this.reset();
            _addCatValue = '';
            updateAddCatBtn();
...
async function fetchThemeSettings(){
    // ⚡ CACHE: Проверяем кеш для theme (skip in Space B — separate space, separate state)
    const cachedTheme = CacheManager.get('theme_settings');
    if (cachedTheme && !isSpaceB()) {
...
async function fetchDisplaySettings(){
    if (isSpaceB()) return;
    try{const response=await makeEncryptedRequest('/api/display_settings');const data=await response.json();document.getElementById('display-timeout').value=data.display_timeout;if(document.getElementById('auto-lock-timeout'))document.getElementById('auto-lock-timeout').value=data.auto_lock_timeout||0;if(document.getElementById('dim-timeout'))document.getElementById('dim-timeout').value=data.dim_timeout||0;const splashResponse=await makeEncryptedRequest('/api/splash/mode');if(splashResponse.ok){const splashData=await splashResponse.json();const selectElement=document.getElementById('splash-mode-select');if(selectElement)selectElement.value=splashData.mode}const clockResponse=await makeEncryptedRequest('/api/clock_settings');if(clockResponse.ok){const clockData=await clockResponse.json();const tzSelect=document.getElementById('timezone-select');if(tzSelect&&clockData.timezone)tzSelect.value=clockData.timezone}await loadRtcStatus();await loadDisplayRotation();await updateDeviceBlePinLabels()}catch(err){showStatus('Error fetching display settings.',true)}}
async function loadBootMode(){
    try{const response=await makeEncryptedRequest('/api/boot-mode');if(!response.ok)return;const data=await response.json();const selectEl=document.getElementById('boot-mode-select');if(selectEl&&data.boot_mode)selectEl.value=data.boot_mode}catch(err){console.warn('Boot mode load failed',err)}}
async function loadHidMode(){const section=document.getElementById('hid-mode-section');if(!section)return;try{const response=await makeEncryptedRequest('/api/hid-mode');if(!response.ok)return;const data=await response.json();const sel=document.getElementById('hid-mode-select');if(sel&&data.hid_mode)sel.value=data.hid_mode;section.style.display=''}catch(err){console.warn('loadHidMode failed',err)}}
async function loadDisplayRotation(){
    if (isSpaceB()) return;
    try{const response=await makeEncryptedRequest('/api/display/rotation');if(!response.ok)return;const data=await response.json();const sel=document.getElementById('display-rotation');if(sel&&data.rotation!==undefined)sel.value=String(data.rotation)}catch(err){console.warn('loadDisplayRotation failed',err)}}
async function saveDisplayRotation(value){try{await makeEncryptedRequest('/api/display/rotation',{method:'POST',body:JSON.stringify({rotation:parseInt(value)}),headers:{'Content-Type':'application/json'}})}catch(err){console.error('saveDisplayRotation:',err)}}
function updateDeviceBlePinLabels(){
    try{
        if (boardHasUsbHid) {
            document.querySelectorAll('[data-i18n^="pin.device."]').forEach(el => {
                const key = el.getAttribute('data-i18n');
                if (key && !key.startsWith('pin.device.hid.')) {
                    el.setAttribute('data-i18n', key.replace('pin.device.', 'pin.device.hid.'));
                }
            });
        } else {
            document.querySelectorAll('[data-i18n^="pin.device.hid."]').forEach(el => {
                const key = el.getAttribute('data-i18n');
                if (key) {
                    el.setAttribute('data-i18n', key.replace('pin.device.hid.', 'pin.device.'));
                }
            });
        }
...
function trDevicePin(key) {
    return tr((typeof boardHasUsbHid !== 'undefined' && boardHasUsbHid) ? 'pin.device.hid.' + key : 'pin.device.' + key);
}
document.getElementById('save-hid-mode-btn').addEventListener('click',async function(){this.disabled=true;this.textContent=tr('settings.hid.saving');try{const mode=document.getElementById('hid-mode-select').value;const response=await makeEncryptedRequest('/api/hid-mode',{method:'POST',body:JSON.stringify({hid_mode:mode}),headers:{'Content-Type':'application/json'}});const data=await response.json();showStatus(data.success?tr('settings.hid.saved'):tr('settings.hid.error'),!data.success)}catch(err){showStatus('Error: '+err,true)}finally{this.disabled=false;this.textContent=tr('settings.hid.btn')}});
...
(function(){const AL_ORDER=[0,300,900,1800,3600,14400];const DIM_ORDER=[0,5,10,15,30,60,300];function enforceLadder(){const dimSel=document.getElementById('dim-timeout');const dspSel=document.getElementById('display-timeout');const alSel=document.getElementById('auto-lock-timeout');let dim=parseInt(dimSel.value);let dsp=parseInt(dspSel.value);let al=parseInt(alSel.value);if(dim>0&&dsp>0&&dim>=dsp){const next=DIM_ORDER.slice().reverse().find(function(v){return v<dsp&&v>0});dimSel.value=next!==undefined?String(next):'0';dim=parseInt(dimSel.value);}if(dsp>0&&al>0&&al<=dsp){const next=AL_ORDER.find(function(v){return v>dsp});alSel.value=next!==undefined?String(next):'0';al=parseInt(alSel.value);}if(dim>0&&al>0&&dim>=al){const next=DIM_ORDER.slice().reverse().find(function(v){return v<al&&v>0});dimSel.value=next!==undefined?String(next):'0';}};document.getElementById('dim-timeout').addEventListener('change',enforceLadder);document.getElementById('display-timeout').addEventListener('change',enforceLadder);document.getElementById('auto-lock-timeout').addEventListener('change',enforceLadder);document.getElementById('display-timeout-form').addEventListener('submit',function(e){e.preventDefault();const dimTimeout=parseInt(document.getElementById('dim-timeout').value);const timeout=parseInt(document.getElementById('display-timeout').value);const autoLock=parseInt(document.getElementById('auto-lock-timeout').value);if(dimTimeout>0&&timeout>0&&dimTimeout>=timeout){showStatus('Dim timeout must be less than screen timeout!',true);return;}if(timeout>0&&autoLock>0&&autoLock<=timeout){showStatus(tr('display.lock.error.order'),true);return;}if(dimTimeout>0&&autoLock>0&&dimTimeout>=autoLock){showStatus('Dim timeout must be less than auto lock timeout!',true);return;}const formData=new FormData();formData.append('dim_timeout',dimTimeout);formData.append('display_timeout',timeout);formData.append('auto_lock_timeout',autoLock);makeEncryptedRequest('/api/display_settings',{method:'POST',body:new URLSearchParams(formData)}).then(res=>res.json()).then(data=>{if(data.success){showStatus(tr('display.lock.saved'))}else{showStatus(data.message||'Error saving settings',true)}}).catch(err=>showStatus('Error saving display settings: '+err,true));});})();
...
async function loadRtcStatus(){if (isSpaceB()) return;
try{const response=await makeEncryptedRequest('/api/rtc');if(!response.ok)return;const data=await response.json();const enabledEl=document.getElementById('rtc-enabled');if(!enabledEl)return;enabledEl.checked=!!data.enabled;const defaultSda=data.default_sda||21;const defaultScl=data.default_scl||22;document.getElementById('rtc-sda').value=data.sda_pin||defaultSda;document.getElementById('rtc-scl').value=data.scl_pin||defaultScl;const boardNote=document.getElementById('rtc-board-note');if(boardNote){boardNote.textContent=(data.board?data.board+' — ':'')+'Default: SDA='+defaultSda+', SCL='+defaultScl;}document.getElementById('rtc-pin-config').style.display=data.enabled?'block':'none';const badge=document.getElementById('rtc-status-badge');const timeDisplay=document.getElementById('rtc-time-display');const statusRow=document.getElementById('rtc-status-row');if(data.enabled){statusRow.style.display='flex';if(!data.available){badge.style.background='#e74c3c';badge.textContent=tr('display.rtc.status.notfound');timeDisplay.textContent=tr('display.rtc.status.check')}else{const rtcTs=data.rtc_time?Math.floor(new Date(data.rtc_time).getTime()/1000):0;const drift=Math.abs(rtcTs-Math.floor(Date.now()/1000));if(drift>3600){badge.style.background='#f39c12';badge.textContent=tr('display.rtc.status.drifted');timeDisplay.textContent=data.rtc_time?new Date(data.rtc_time).toLocaleString([],{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}):''}else{badge.style.background='#27ae60';badge.textContent=tr('display.rtc.status.ok');timeDisplay.textContent=data.rtc_time?new Date(data.rtc_time).toLocaleString([],{year:'numeric',month:'2-digit',day:'2-digit',hour:'2-digit',minute:'2-digit',second:'2-digit',hour12:false}):''}}}else{statusRow.style.display='none'}document.getElementById('rtc-sync-hint').textContent=data.wifi_connected?tr('display.rtc.hint.ntp'):tr('display.rtc.hint.browser')}catch(err){console.warn('RTC status load failed',err)}}
...
async function fetchPinSettings(){
    ...
    if (cachedPin) {
        ...
        boardHasUsbHid = !!cachedPin.has_usb_hid;
        updateDeviceBlePinLabels();
        document.getElementById('pin-enabled-device').checked = cachedPin.enabledForDevice;
        
        // Загружаем Device BLE PIN status
        if (cachedPin.length && cachedPin.length >= 4 && cachedPin.length <= 10) {
            devicePinLength = cachedPin.length;
            var dp = document.getElementById('duress-pin');
            var dpc = document.getElementById('duress-pin-confirm');
            if (dp)  { dp.maxLength = devicePinLength; dp.pattern = '\\d{' + devicePinLength + '}'; }
            if (dpc) { dpc.maxLength = devicePinLength; dpc.pattern = '\\d{' + devicePinLength + '}'; }
        }
        ...
        if (cachedPin.duressPinEnabled !== undefined) {
            document.getElementById('duress-pin-enabled').checked = cachedPin.duressPinEnabled;
            toggleDuresspinFields();
        }
        
        // Load Hidden Space status
        fetchHiddenSpaceStatus();
    ...
    const response = await makeEncryptedRequest('/api/pincode_settings', {
        ...
        console.log('📌 PIN Settings loaded:', {
            ...
            has_usb_hid: data.has_usb_hid
        });
        
        boardHasUsbHid = !!data.has_usb_hid;
        updateDeviceBlePinLabels();
        ...
        if (data.duressPinEnabled !== undefined) {
            document.getElementById('duress-pin-enabled').checked = data.duressPinEnabled;
            toggleDuresspinFields();
        }
...
function toggleDuresspinFields() {
    var enabled = document.getElementById('duress-pin-enabled').checked;
    var fields = document.getElementById('duress-pin-fields');
    if (!fields) return;
    fields.style.display = enabled ? 'block' : 'none';
    if (!enabled) {
        document.getElementById('duress-pin').value = '';
        document.getElementById('duress-pin-confirm').value = '';
        var cached = CacheManager.get('pin_settings');
        if (cached && cached.duressPinConfigured && cached.duressPinEnabled) {
            var fd = new FormData();
            fd.append('duress_pin_enabled', 'false');
            makeEncryptedRequest('/api/duress_pin_update', {method:'POST', body:fd})
                .then(function(r){return r.json();})
                .then(function(d){
                    if (d.success) {
                        showStatus(tr('pin.duress.disabled'));
                        CacheManager.invalidate('pin_settings');
                        fetchPinSettings();
                    } else {
                        showStatus(d.message || 'Error disabling Duress PIN', true);
                        document.getElementById('duress-pin-enabled').checked = true;
    ...
    if (!enabled) {
        ...
        if (cachedPin && cachedPin.deviceBlePinConfigured && cachedPin.deviceBlePinEnabled) {
            ...
            makeEncryptedRequest('/api/ble_pin_update', {method: 'POST', body: formData})
                ...
                .then(data => {
                    if (data.success) {
                        showStatus(trDevicePin('disabled'));
                        CacheManager.invalidate('pin_settings');
                        fetchPinSettings();
                    } else {
...
// Hidden Space Management Functions
async function fetchHiddenSpaceStatus() {
    try {
        const res = await makeEncryptedRequest('/api/hidden_space', {
            method: 'GET',
            headers: { 'X-User-Activity': 'true' }
        });
        
        if (!res.ok) {
            console.error('Failed to fetch hidden space status');
            return;
        }
        
        const data = await res.json();
        
        // ⚡ CACHE: Store hidden space status for PIN disable warning
        CacheManager.set('hidden_space_status', data);
        
        const toggle = document.getElementById('hidden-space-enabled');
        const hint   = document.getElementById('hidden-space-hint');
        const btn    = document.getElementById('hidden-space-action-btn');
        const block  = document.getElementById('hidden-space-confirm-block');
        
        if (!toggle || !hint || !btn || !block) return;
        
        toggle.checked = data.hidden_space_enabled;
        
        // Share WiFi elements
        const shareRow = document.getElementById('share-wifi-row');
        const shareHint = document.getElementById('share-wifi-hint');
        const shareCheck = document.getElementById('share-wifi-enabled');
        
        if (data.current_space === 'B') {
            // In Space B: disable the toggle (can't create nested hidden space)
            toggle.disabled = true;
            hint.setAttribute('data-i18n', 'hidden_space_hint_space_b');
            if (data.hidden_space_enabled) {
                // Show disable button only
                block.style.display = 'block';
                btn.setAttribute('data-i18n', 'hidden_space_disable_btn');
                btn.className = 'button user-activity';
                btn.style.backgroundColor = '#e74c3c';
                btn.setAttribute('data-action', 'disable');
            } else {
                block.style.display = 'none';
            }
            // Hide share-wifi in Space B
            if (shareRow && shareHint) {
                shareRow.style.display = 'none';
                shareHint.style.display = 'none';
            }
            // Hide entire hidden space section in Space B
            const hiddenSpaceSection = document.getElementById('hidden-space-section');
            if (hiddenSpaceSection) hiddenSpaceSection.style.display = 'none';
        } else {
            // Space A context
            toggle.disabled = false;
            hint.setAttribute('data-i18n', 'hidden_space_hint_space_a');
            
            // Show hidden space section in Space A
            const hiddenSpaceSection = document.getElementById('hidden-space-section');
            if (hiddenSpaceSection) hiddenSpaceSection.style.display = 'block';
            
            if (data.hidden_space_enabled) {
                // Hidden space ON: show disable button, hide warning block
                block.style.display = 'none';
                btn.setAttribute('data-i18n', 'hidden_space_disable_btn');
                btn.textContent = tr('hidden_space_disable_btn') || 'Disable Hidden Space';
                btn.className = 'button user-activity';
                btn.style.backgroundColor = '#e74c3c';
                btn.setAttribute('data-action', 'disable');
                
                // Show WiFi share checkbox
                if (shareRow && shareHint && shareCheck) {
                    shareRow.style.display = 'flex';
                    shareHint.style.display = 'block';
                    shareCheck.checked = data.share_wifi_enabled || data.share_wifi || false;
                }
            } else {
                // Hidden space OFF: show enable button, hide warning and share row
                block.style.display = 'none';
                btn.setAttribute('data-i18n', 'hidden_space_enable_btn');
                btn.textContent = tr('hidden_space_enable_btn') || 'Enable & Setup';
                btn.className = 'button user-activity';
        ...
        // ═══ HIDE DURESS PIN SECTION IN SPACE B ═══
        // Duress PIN is Space A prerogative — duress wipe destroys Space B too
        const duressSection = document.getElementById('duress-pin-section');
        if (duressSection) {
            duressSection.style.display = (data.current_space === 'B') ? 'none' : 'block';
        }
        
        // ═══ HIDE REBOOT WITH WEB SERVER BUTTON IN SPACE B ═══
        // This button sets web_server_auto_start in global /config.json
        // Triggering from Space B affects Space A boot behavior
        const rebootWebBtn = document.getElementById('reboot-with-web-btn');
        if (rebootWebBtn) {
            rebootWebBtn.style.display = (data.current_space === 'B') ? 'none' : '';
        }
        
        // ═══ HIDE SPLASH SETTINGS IN SPACE B ═══
        // splash-mode-select and save-splash-mode-btn modify global /splash_config.json
        // Changes from Space B affect Space A boot appearance
        const splashSelect = document.getElementById('splash-mode-select');
        const splashBtn    = document.getElementById('save-splash-mode-btn');
        const splashDisplay = (data.current_space === 'B') ? 'none' : '';
        if (splashSelect) splashSelect.style.display = splashDisplay;
        if (splashBtn)    splashBtn.style.display    = splashDisplay;
        
        // ═══ HIDE GLOBAL DEVICE SETTINGS IN SPACE B ═══
        const globalEls = [
            'pin-startup-section',
            'rtc-section',
            'wifi-password-toggle',
            'web-server-settings-form',
            'display-rotation',
            'dim-timeout',      'display-timeout',
            'auto-lock-timeout','display-timeout-form',
            'session-duration', 'session-duration-form',
            'mdns-settings-form', 'mdns-hostname',
            'ble-settings-form',  'ble-device-name'
        ];
        globalEls.forEach(id => {
            const el = document.getElementById(id);
            if (el) el.style.display = (data.current_space === 'B') ? 'none' : '';
        });
        
        applyLang(currentLang, true); // refresh translations
    } catch (err) {
        console.error('Error fetching hidden space status:', err);
    }
}

function toggleHiddenSpaceField() {
    const enabled   = document.getElementById('hidden-space-enabled').checked;
    const block     = document.getElementById('hidden-space-confirm-block');
    const btn       = document.getElementById('hidden-space-action-btn');
    const shareRow  = document.getElementById('share-wifi-row');
    const shareHint = document.getElementById('share-wifi-hint');
    ...
    const cachedHS        = CacheManager.get('hidden_space_status');
    const alreadyExists   = cachedHS && cachedHS.hidden_space_enabled;
    
    if (enabled) {
        if (alreadyExists) {
            // Hidden space exists — show disable button + wifi share
            btn.setAttribute('data-i18n', 'hidden_space_disable_btn');
            btn.textContent = tr('hidden_space_disable_btn') || 'Disable Hidden Space';
            btn.className = 'button user-activity';
            btn.style.backgroundColor = '#e74c3c';
            ...
            if (shareCheck && cachedHS)
                shareCheck.checked = cachedHS.share_wifi_enabled || false;
        } else {
            // First-time enable — show warning, hide wifi share
            btn.setAttribute('data-i18n', 'hidden_space_enable_btn');
            btn.textContent = tr('hidden_space_enable_btn') || 'Enable & Setup';
            btn.className = 'button user-activity';
            btn.style.backgroundColor = '#27ae60';
        ...
        if (alreadyExists) {
            // Hidden space exists — confirm disable
            btn.setAttribute('data-i18n', 'hidden_space_disable_btn');
            btn.textContent = tr('hidden_space_disable_btn') || 'Disable Hidden Space';
            btn.className = 'button user-activity';
            btn.style.backgroundColor = '#e74c3c';
            ...
            // Nothing to disable — enable button
            btn.setAttribute('data-i18n', 'hidden_space_enable_btn');
            btn.textContent = tr('hidden_space_enable_btn') || 'Enable & Setup';
            btn.className = 'button user-activity';
            btn.style.backgroundColor = '#27ae60';
...
async function submitHiddenSpaceAction() {
    const toggle  = document.getElementById('hidden-space-enabled');
    const btn = document.getElementById('hidden-space-action-btn');
    const action = btn.getAttribute('data-action') || 'enable';
    const confirmMsg = action === 'enable'
        ? (tr('hidden_space_confirm_enable') || 'Enable Hidden Space? Device will reboot and create Space B.')
        : (tr('hidden_space_confirm_disable') || 'Disable Hidden Space? All data in Space B will be wiped and device will reboot.');
    ...
    try {
        ...
        if (data.status === 'rebooting' || data.status === 'wiped_rebooting') {
            showStatus(tr('hidden_space_rebooting') || 'Device is rebooting...', false);
        } else if (data.status === 'disabled') {
            showStatus(tr('hidden_space_disabled_ok') || 'Hidden Space disabled successfully', false);
            fetchHiddenSpaceStatus();
        } else if (data.error) {
            showStatus(tr('error_' + data.error) || ('Error: ' + data.error), true);
            fetchHiddenSpaceStatus();
        }
    } catch (err) {
        showStatus('Error: ' + err.message, true);
...
async function submitShareWifiChange() {
    const enabled = document.getElementById('share-wifi-enabled').checked;
    ...
    try {
        const res = await makeEncryptedRequest('/api/hidden_space', {
            ...
            body: JSON.stringify({ action: 'set_share_wifi', enabled: enabled })
        });
        
        if (!res.ok) {
            showStatus(tr('error_generic') || 'Error', true);
            fetchHiddenSpaceStatus(); // revert UI
            return;
        }
        
        const data = await res.json();
        const key = enabled ? 'hidden_space_wifi_shared_ok' : 'hidden_space_wifi_unshared_ok';
        showStatus(tr(key) || (enabled ? 'WiFi shared' : 'WiFi unshared'), false);
    } catch (err) {
        showStatus('Error: ' + err.message, true);
        fetchHiddenSpaceStatus(); // revert UI
    }
...
document.getElementById('pincode-settings-form').addEventListener('submit', async function(e) {
    ...
    if (enabledForDevice !== wasEnabled) {
        ...
        if (enabledForDevice) {
            ...
            // Check if secondary slot exists
            const cachedHiddenSpace = CacheManager.get('hidden_space_status');
            if (cachedHiddenSpace && cachedHiddenSpace.hidden_space_enabled) {
                warningMessage += '🚨 CRITICAL:\n' +
                                '• Secondary slot will be PERMANENTLY DELETED\n' +
                                '• All secondary slot data will be LOST\n' +
...
document.getElementById('duress-pin-form').addEventListener('submit', function(e) {
    e.preventDefault();
    var enabled = document.getElementById('duress-pin-enabled').checked;
    var pin = document.getElementById('duress-pin').value;
    var pinConfirm = document.getElementById('duress-pin-confirm').value;
    if (!enabled) { showStatus(tr('pin.duress.error.enable'), true); return; }
    if (pin.length !== devicePinLength || !/^\d+$/.test(pin)) {
        showStatus(tr('pin.duress.error.digits'), true); return;
    }
    if (pin !== pinConfirm) { showStatus(tr('pin.duress.error.match'), true); return; }
    var fd = new FormData();
    fd.append('duress_pin_enabled', 'true');
    fd.append('duress_pin', pin);
    makeEncryptedRequest('/api/duress_pin_update', {method:'POST', body:fd})
        .then(function(r){return r.json();})
        .then(function(d){
            if (d.success) {
                showStatus(tr('pin.duress.saved'));
                document.getElementById('duress-pin').value = '';
                document.getElementById('duress-pin-confirm').value = '';
                CacheManager.invalidate('pin_settings');
                fetchPinSettings();
            } else {
                showStatus(d.message || 'Error updating Duress PIN', true);
            }
        })
...
document.getElementById('device-ble-pin-form').addEventListener('submit',function(e){
    ...
    if(!enabled){
        showStatus(trDevicePin('error.enable'),true);
        return;
    }
    
    if(deviceBlePin.length!==6||!/^\d{6}$/.test(deviceBlePin)){
        showStatus(trDevicePin('error.digits'),true);
        return;
    }
    if(deviceBlePin!==deviceBlePinConfirm){
        showStatus(trDevicePin('error.match'),true);
        return;
    }
...
document.getElementById('clear-ble-clients-btn').addEventListener('click',function(){
    // Check if we're in Space B to show global warning
    const cachedHiddenSpace = CacheManager.get('hidden_space_status');
    const isSpaceB = (cachedHiddenSpace && cachedHiddenSpace.current_space === 'B');
    
    const confirmMsg = isSpaceB
        ? (i18n[currentLang]?.['ble_clear_confirm_global']
           || 'Clear ALL BLE pairings? This affects both spaces — main and hidden — and cannot be undone.')
        : (i18n[currentLang]?.['ble_clear_confirm']
           || 'Clear all BLE client connections? This will remove all paired devices and they will need to pair again.');
    
    if(!confirm(confirmMsg)){return}
    
    const formData=new FormData();
    makeEncryptedRequest('/api/clear_ble_clients',{method:'POST',body:formData})
        .then(res=>res.json())
        .then(data=>{
            if(data.success){
                showStatus(tr('settings.system.ble.cleared'))
            }else{
                showStatus(data.message||'Failed to clear BLE clients',true)
            }
        })
        .catch(err=>showStatus('Error clearing BLE clients: '+err,true))
});


async function fetchStartupMode(){
    if (isSpaceB()) return;
    try{const response=await makeEncryptedRequest('/api/startup_mode');const data=await response.json();document.getElementById('startup-mode').value=data.mode}catch(err){showStatus('Error fetching startup mode.',true)}}
...
const passwordCriteria = {
    ...
    noUsername: { el: document.getElementById('pwd-no-username'), test: null }
};

function validateNewPassword() {
    const password = document.getElementById('new-password').value;
    let allValid = true;
    const rawUsername = (document.getElementById('current-admin-login')?.textContent || '').trim().toLowerCase();
    const pwLower = password.toLowerCase();
    const usernameWords = rawUsername.match(/[a-z]{3,}/g) || [];
    passwordCriteria.noUsername.test = () =>
        usernameWords.length === 0 || !usernameWords.some(w => pwLower.includes(w));
    
    for (const key in passwordCriteria) {
        const crit = passwordCriteria[key];
        const isValid = crit.regex ? crit.regex.test(password) : (crit.test ? crit.test() : true);
        crit.el.classList.toggle('valid', isValid);
...
async function fetchBleSettings(){
    if (isSpaceB()) return;
    try{const response=await makeEncryptedRequest('/api/ble_settings');const data=await response.json();document.getElementById('ble-device-name').value=data.device_name}catch(err){showStatus('Error fetching BLE settings.',true)}}
...
async function fetchMdnsSettings(){
    if (isSpaceB()) return;
    try{const response=await makeEncryptedRequest('/api/mdns_settings');const data=await response.json();document.getElementById('mdns-hostname').value=data.hostname}catch(err){showStatus('Error fetching mDNS settings.',true)}}
...
const CATEGORIES = [
    { id: '',      emoji: '📋', labelKey: 'passwords.cat.all'   },
    { id: 'web',   emoji: '🌐', labelKey: 'passwords.cat.web'   },
    { id: 'app',   emoji: '📱', labelKey: 'passwords.cat.app'   },
    { id: 'local', emoji: '🖥',  labelKey: 'passwords.cat.local' },
    { id: 'key',   emoji: '🔑', labelKey: 'passwords.cat.key'   },
];
...
function getCatBadgeClass(catId) {
    const map = { web: 'badge-cat-web', app: 'badge-cat-app',
                  local: 'badge-cat-local', key: 'badge-cat-key' };
    return map[catId] || 'badge-cat-none';
}

function catBadgeHTML(catId, clickable = true) {
    const cat = getCatInfo(catId);
    const cls = getCatBadgeClass(catId);
    const label = catId ? tr(cat.labelKey) : tr('passwords.cat.none');
    const onclick = clickable ? `onclick="openCatPopup(event, this)" data-cat="${catId || ''}"` : '';
    return `<span class="badge badge-category ${cls}" ${onclick}>${cat.emoji} ${label}</span>`;
}
...
class SecureClient {
    ...
    shouldTunnelEndpoint(endpoint) {
        ...
        const tunneledEndpoints = [
            ...
            '/api/display/rotation',  // ✅ GET/POST - ориентация экрана
            ...
            '/api/hid-mode',          // ✅ GET/POST - выбор режима HID (BLE/USB)
            '/api/hidden_space',      // ✅ GET/POST - Hidden Space Management
        ...
        '/api/duress_pin_update', // ✅ POST - Duress PIN update
    ...
    processHeadersWithObfuscation(headers, endpoint, method) {
        ...
        // D) Header Payload Embedding - embed random noise (not real endpoint/method)
        // Real metadata must never appear here — it would defeat URL obfuscation and method tunneling
        const fakeMetadata = {
            sid: generateRandomToken(16, 'hex'),
            v: Math.floor(Math.random() * 900 + 100),
            ts: Date.now() - Math.floor(Math.random() * 60000)
        };
        const encodedData = btoa(JSON.stringify(fakeMetadata));
    ...
    shouldSecureEndpoint(url) {
        ...
        '/api/duress_pin_update',  // 🔐 Duress PIN update (critical security)
            ...
            '/api/display/rotation',   // 🔐 Display rotation settings (NEW)
            ...
            '/api/hid-mode',           // 🔐 HID mode selection (BLE/USB) (NEW)
            '/api/hidden_space',       // 🔐 Hidden Space Management (NEW)
...
function reorderItems(dataType, fromIndex, toIndex) {
    if (dataType === 'keys') {
        ...
        const pwdTbody = document.querySelector('#passwords-table tbody');
        if (pwdTbody) {
            [...pwdTbody.querySelectorAll('.draggable-row')].forEach((row, i) => {
                row.dataset.index = i;
            });
        }
...
document.addEventListener('DOMContentLoaded', async function(){
    try {
        ...
        try {
            ...
            let batteryPingInFlight = false;
            async function updateBatteryWidgetGuarded() {
                if (batteryPingInFlight) return;
                batteryPingInFlight = true;
                try { await updateBatteryWidget(); } finally { batteryPingInFlight = false; }
            }
            setInterval(updateBatteryWidgetGuarded, 30000);
...
function enableApi() {
    ...
    .then(res => {
        if (res.ok) {
            ...
            async function updateApiStatusGuarded() {
                if (apiStatusInFlight) return;
                apiStatusInFlight = true;
                try { await updateApiStatus(); } finally { apiStatusInFlight = false; }
            }
            apiAccessInterval = setInterval(updateApiStatusGuarded, 30000); // Каждые 30 сек
...
(function() {
    ...
    function resetServerActivity() {
        ...
        if (WARNING_TIME > 0) {
            if (!activityPingInFlight) {
                activityPingInFlight = true;
                makeAuthenticatedRequest('/api/activity', { method: 'POST' })
                    .catch(err => console.error("Failed to reset activity timer:", err))
                    .finally(() => { activityPingInFlight = false; });
            }
